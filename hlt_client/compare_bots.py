...
def _determine_winner(game_result):
    ...
    winLine = next(line for line in game_result.splitlines() if re.compile(_WINNING_RANK_STRING).search(line))
    
    if "dealing 0 damage" in winLine:
        return "TIES"
	
    return winLine.split(_SPACE_DELIMITER)[_BOT_ID_POSITION]
