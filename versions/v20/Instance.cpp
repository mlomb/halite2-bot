			...
			if (ship->IsOur() && !ship->IsCommandable()) {
				...
				if (distance < 35) {
					...
					if (taskDefend == 0) {
						...
						int enemyShips = CountNearbyShips(enemyShip->location, enemyShip->radius, 7, false);

						taskDefend->max_ships = 1; //std::max(1, std::min(3, enemyShips)); // max shouldnt be necessary
					...
					if (distance < taskDefend->defendingDistance) {
						...
						double angle_rad = ship->location.OrientTowardsRad(enemyShip->location);
						double magnitude = distance * 0.2;

						taskDefend->location = ship->location + Vector2{ cos(angle_rad) * magnitude, sin(angle_rad) * magnitude };
					}
				}
			}
		}
