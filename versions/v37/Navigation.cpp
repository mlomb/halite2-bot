...
static void CalculateDesiredMovement(std::set<NavigationRequest*>& navigationRequests) {
	...
	for (; it != navigationRequests.end();) {
		...
		if (navReq->avoid_obstacles) {
			...
			navReq->desiredMovement = NavigationState(navReq->ship->location.OrientTowardsRad(navReq->targetLocation), hlt::constants::MAX_SPEED);
			navReq->currentMovement = navReq->desiredMovement;
			++it;
			continue;
		}
...
{
	if (avoid_obstacles && conflicts.size() > 0)
		return true;
	...
	if (avoid_obstacles && Navigation::AreObjectsBetween(ship->location, futurePosition))
		return true;
