...
std::vector<Move> Instance::Frame()
{
	...
	if (rush_phase) { // we check if the rush phase should end
		...
		if (!threatsFound) {
			if ((planetsCount.at(player_id) >= 1 && shipsCount.at(player_id) >= 4) || turn > 30) {
				// just end the rushing detection phase if we generated at least 4 ships with a planet
				// or turn > 30
				Log::log("Ending the rush phase because we have at least 4 ships or turn > 30");
				threatsFound = false;
			}
		}
...
void Instance::GenerateTasks()
{
	...
	if (!game_over) {
		...
		for (auto& kv : ships) {
			...
			if (num_players == 4 && turn > 50 && planetsCount.at(player_id) > 0 && planetsCount.at(ship->owner_id) == 0) {
				...
				for (auto& kv2 : ships) {
					...
					if (ship2->IsOur() && !ship2->IsCommandable()) {
						if (ship2->location.DistanceTo(ship->location) < 30) {
							is_attacking = true;
							break;
						}
					}
			...
			int enemyShips = CountNearbyShips(ship->location, ship->radius, 7, false);

			taskAttack->max_ships = 1 + ceil(enemyShips / 5.0);
		}

		// Defend tasks
		for (auto& kv : planets) {
			Planet* planet = kv.second;
			if (!planet->IsOur()) continue;

			Vector2 indefenseShipAverage = { 0, 0 };
			for (Ship* dockedShip : planet->docked_ships)
				indefenseShipAverage = indefenseShipAverage + dockedShip->location;
			indefenseShipAverage = indefenseShipAverage / planet->docked_ships.size();

			int enemyShips = CountNearbyShips(indefenseShipAverage, 0, 15, false);
			if (enemyShips > 0) {
				Task* taskAttack = CreateTask(TaskType::DEFEND);
				taskAttack->target = -1;
				taskAttack->location = indefenseShipAverage;
				taskAttack->radius = 0;

				taskAttack->max_ships = enemyShips;
			}
		}
...
void Instance::AssignTasks()
{
	...
	while (!qShips.empty()) {
		...
		// for unsuitable ships
		Task* attackTask = 0;
		double maxAttackPriority = -INF;
		...
		for (Task* task : tasks) {
			...
			if (task->type == ATTACK) {
				if (priority > maxAttackPriority) {
					maxAttackPriority = priority;
					attackTask = task;
				}
			}
		...
		if (priorizedTask == 0) {
			Log::log("Ship " + std::to_string(ship->entity_id) + " couldn't find a suitable task, assigning the closest ATTACK task.");
			priorizedTask = attackTask;
		}
