	...
	while (!qShips.empty()) {
		...
		for (Task* task : tasks) {
			...
			case DOCK:
			{
				...
				offsetPriority += (planetDistanceFromCenter / (double)min_side) * 15;
				...
				if (distance < task->radius + 10) {
					...
					if (enemies != 0 && enemies > friends - 1) {
						offsetPriority -= 1000; // we should not dock
					}
				}
				break;
			}
			case DEFEND:
				offsetPriority += 30; // very very important
				break;
			case ATTACK:
				if (task->indefense) {
					/*
					...
					*/
					...
					offsetPriority += 20 / (enemiesClose + 1);
				}
			...
			double priority = (10000 - distance) + offsetPriority;
			/* PRIORITY CALCULATION */
