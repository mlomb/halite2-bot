...
std::pair<possibly<Move>, possibly<NavigationRequest>> Ship::ComputeAction()
{
...

	{
		...
		case DOCK:
		{
			...
			if (docking_status == ShipDockingStatus::Docked) {
				...
				if (threatened || instance->game_over) {
					return { { Move::undock(entity_id), true }, { { Vector2(), false }, false } };
				}
			...
			if (CanDock(planet)) {
				if (!threatened && !instance->game_over) {
					return { { Move::dock(entity_id, planet->entity_id), true },{ { Vector2(), false }, false } };
				}
				else {
		...
		Log::log() << "Ship " << entity_id << " is moving towards " << navRequest.targetLocation << " avoiding enemies: " << navRequest.avoid_enemies << std::endl;
		if (navRequest.targetLocation.DistanceTo(location) < 1) {
			Log::log() << "... but ship it's already there so... we do nothing" << std::endl;
			goto nomove;
		}
...
nomove:
	return { { Move::noop(), false },{ { Vector2(), false }, false } };
}
