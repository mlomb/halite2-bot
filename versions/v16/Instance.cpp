...
std::vector<Move> Instance::Frame()
{
	...
	std::vector<Move> moves;
	std::set<NavigationRequest*> navigationRequests;

	{
		...
		for (Ship* ship : myShips) {
			...
			if (action.second.second) {
				ship->frozen = false;
				navigationRequests.insert(action.second.first);
			}
			else {
...
void Instance::GenerateTasks()
{
	...
	if (!game_over) {
		...
		for (auto& kv : ships) {
			...
			if (num_players == 4 && turn > 50 && planetsCount.at(player_id) > 0 && planetsCount.at(ship->owner_id) == 0) {
				...
				for (auto& kv2 : ships) {
					...
					if (ship2->IsOur() && !ship2->IsCommandable()) {
						if (ship2->location.DistanceTo(ship->location) < 15) {
							is_attacking = true;
							break;
						}
					}
			...
			taskAttack->max_ships = 1 + ceil(enemyShips / 4.0);// TODO See
		}
		...
		if (writing) {
			...
			Vector2 messageOffset = { map_width / 2.0 - messageSize.x / 2.0, map_height / 2.0 + middlePlanetRadius * 2 + 7 };
