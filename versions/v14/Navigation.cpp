...
static void AlignShips(std::vector<NavigationRequest*>& navigationRequests) {
...
std::vector<Move> Navigation::NavigateShips(std::vector<NavigationRequest*> navigationRequests)
{
	...
	for (auto it1 = navigationRequests.begin(); it1 != navigationRequests.end(); it1++) {
		...
		for (auto it2 = it1 + 1; it2 != navigationRequests.end(); it2++) {
			NavigationRequest* navReqB = *it2;

			if (navReqA->ship->location.DistanceTo(navReqB->ship->location) < (hlt::constants::SHIP_RADIUS * 2 + hlt::constants::MAX_SPEED) * 2) {
				navReqA->eventHorizon.push_back(navReqB);
				navReqB->eventHorizon.push_back(navReqA);
			}
		}
	}
	...
	while (!q.empty()) {
		...
		if (conflict) {
			...
			for (NavigationRequest* navReqOther : navReq->eventHorizon) {
				navReqOther->eventHorizon.erase(std::find(navReqOther->eventHorizon.begin(), navReqOther->eventHorizon.end(), navReq));
			}
...
{
	const double r = hlt::constants::SHIP_RADIUS * 2 + 0.001;
