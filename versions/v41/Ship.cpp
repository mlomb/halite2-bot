...
std::pair<possibly<Move>, possibly<NavigationRequest*>> Ship::ComputeAction()
{
...

	{
		...
		case ATTACK:
		{
			...
			if (shipTarget->docked_planet >= 0) { // docked ships
				Planet* docked_planet = instance->GetPlanet(shipTarget->docked_planet);
				Vector2 point = shipTarget->location + Vector2::Velocity(docked_planet->location.OrientTowardsRad(shipTarget->location), 10);
				navRequest->targetLocation = point.ClosestPointTo(shipTarget->location, shipTarget->radius, 1);
			}
			else {
				navRequest->targetLocation = location.ClosestPointTo(shipTarget->location, shipTarget->radius, hlt::constants::MIN_DISTANCE_FOR_CLOSEST_POINT );
			}
