...
possibly<Vector2> Navigation::GetNavigationTargetTowardsTarget(const Vector2& location, const Vector2& target)
{
	...
	for (int i = 0; i < 360 * 2; i++) {
		...
		if (!AreObjectsBetween(location, new_target)) {
...
void Navigation::CalculateMovementPoint(NavigationRequest* navReq) {
	...
	if (navReq->avoid_enemies) {
		...
		if (navReq->ship->task_id != -1)
			defending = instance->GetTask(navReq->ship->task_id)->type == TaskType::DEFEND;

		double minDist = INF;
		int maxEnemies = 0;
		bool foundAttackPoint = false;

		map->IterateMap(navReq->ship->location, movementRadius + 1 /* one unit more */, [&](Vector2 position, MapCell& cell, double distance) {
			if (cell.solid) return;

			bool good;
			if (defending)
				good = cell.nextTurnFriendlyShipsTakingDamage >= cell.nextTurnEnemyShipsAttackInRange;
			else
				good = cell.nextTurnFriendlyShipsTakingDamage > cell.nextTurnEnemyShipsAttackInRange;

			if (good) {
				if ((defending || cell.nextTurnEnemyShipsTakingDamage >= maxEnemies)) {
	...
	for (NavigationRequest* navReqA : navigationRequests) {
		if (navReqA->cluster_id == -1) {
			...
			for (NavigationRequest* navReqB : navigationRequests) {
				Task* taskA = instance->GetTask(navReqA->ship->task_id);
				Task* taskB = instance->GetTask(navReqB->ship->task_id);
			...
			if (cluster.size() > 1) {
				...
				for (NavigationRequest* navReqCluster : cluster) {
					navReqCluster->desiredMovement = NavigationState(averageAngle, navReqCluster->desiredMovement.thrust);
...
std::vector<Move> Navigation::NavigateShips(std::set<NavigationRequest*> navigationRequests)
{
	...
	/*
	{
		Stopwatch s("Navigation test");
		std::vector<NavigationState*> states;
		states.reserve(navigationRequests.size() * 7 * 178);
		for (NavigationRequest* navReq : navigationRequests) {
			navReq->states.reserve(7 * 178);
			for (int angle_deg = 1; angle_deg <= 179; angle_deg++) {
				for (int thrust = 7; thrust > 0; thrust--) {
					double angle_rad = navReq->desiredMovement.angle_rad + ((angle_deg - 45) * M_PI) / 180.0;
					NavigationState* state = new NavigationState(angle_rad, thrust);
					state->navReq = navReq;
					navReq->states.emplace_back(state);
					states.emplace_back(state);
				}
			}
		}

		for (NavigationState* navState : states) {
			for (NavigationState* navStateOther : states) {
				if (navState->navReq != navStateOther->navReq) {
					if (navState->Conflict(navStateOther)) {
						navState->conflicts.insert(navStateOther);
					}
				}
			}
		}
	...
	while (!q.empty()) {
		...
		for (int t = navReq->desiredMovement.thrust; conflict && t > 0; t--) {
			for (int i = 1; conflict && i < seekDegrees * 2; i++) {
				...
				angle_rad = (radToDegClipped(angle_rad) * M_PI) / 180.0;
...
bool NavigationRequest::HasConflicts()
{
	...
	if (Navigation::AreObjectsBetween(ship->location, futurePosition))
		return true;
...
bool NavigationState::Conflict(const NavigationState* other) const
{
	const double r = hlt::constants::SHIP_RADIUS * 2;
	auto t = Navigation::collision_time(r, navReq->ship->location, other->navReq->ship->location, velocity, other->velocity);
	return t.first && t.second >= 0 && t.second <= 1;
}
