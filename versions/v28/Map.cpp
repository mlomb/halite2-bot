...
void Map::ModifyShip(Ship* ship, int direction)
{
	...
	if (ship->IsOur()) {
		radius = ship->radius + hlt::constants::MAX_SPEED + hlt::constants::WEAPON_RADIUS;
		...
		if (ship->IsCommandable())
			radius = hlt::constants::SHIP_RADIUS + hlt::constants::MAX_SPEED + hlt::constants::WEAPON_RADIUS + 1 + hlt::constants::SHIP_RADIUS;
		else
	...
	int max_thrusts[360];
	if (ship->IsOur() && !ship->frozen && !is_docking) {
		for (int angle_deg = 0; angle_deg < 360; angle_deg++) {
			max_thrusts[angle_deg] = 7;
			while (max_thrusts[angle_deg] > 0) {
				if (!Navigation::AreObjectsBetween(ship->location, ship->location + Vector2::Velocity((angle_deg * M_PI) / 180.0, max_thrusts[angle_deg])))
					break;
	...
	IterateMap(ship->location, radius, [&](Vector2 position, MapCell& cell, double distance) {
		...
		if (ship->IsOur()) {
			if (ship->frozen || is_docking) {
				...
				if (distance < hlt::constants::SHIP_RADIUS + thrust_distance[max_thrusts[angle_deg]]) {
					cell.nextTurnFriendlyShipsTakingDamage += direction;
					if (ship->IsCommandable()) {
						if (distance < hlt::constants::SHIP_RADIUS + thrust_distance[max_thrusts[angle_deg]] + hlt::constants::WEAPON_RADIUS) {
