...
void Instance::ParseMap(const std::string& input) {
	...
	for (auto& kv : ships) { kv.second->alive = false; kv.second->task_id = -1; kv.second->task_priority = 0; }
...
void Instance::UpdateMap()
{
	...
	for (auto&kv : planets) {
		...
		IterateMap(planet->location, planet->radius, [&](Vector2 position, MapCell& cell, double distance) {
			cell.solid = true;
		});
	}
	...
	for (auto&kv : ships) {
		...
		if (ship->IsOur()) {
			...
			if (ship->IsCommandable()) 
				radius = ship->radius * 2 + hlt::constants::MAX_SPEED + hlt::constants::WEAPON_RADIUS;
			else
...
void Instance::IterateMap(Vector2 location, double radius, std::function<void(Vector2, MapCell&, double)> action) {
	...
	startPoint.x = std::fmin(std::fmax(startPoint.x, borderSeparation), MAP_MAX_WIDTH - borderSeparation);
	startPoint.y = std::fmin(std::fmax(startPoint.y, borderSeparation), MAP_MAX_HEIGHT - borderSeparation);

	endPoint.x = std::fmin(std::fmax(endPoint.x, borderSeparation), MAP_MAX_WIDTH - borderSeparation);
	endPoint.y = std::fmin(std::fmax(endPoint.y, borderSeparation), MAP_MAX_HEIGHT - borderSeparation);
	...
	for (double ix = startPoint.x; ix < endPoint.x; ix += step) {
		for (double iy = startPoint.y; iy < endPoint.y; iy += step) {
			...
			int x = ix * MAP_DEFINITION;
			int y = iy * MAP_DEFINITION;
			if (d < radius) {
				// this verifications shouldnt happen, but they still happen for some reason :(
				if (x < 0 || y < 0 || x >= MAP_WIDTH || y >= MAP_HEIGHT)
					continue;
				action(position, map[y][x], d);
			}
		}
	}
}
...
std::vector<Move> Instance::Frame()
{
	...
	for (Ship* ship : requireComputeShips) {
		auto move = ship->ComputeMove();
		if (move.second) {
			// Add a message in the angle
			//move.first.move_angle_deg += ((GetTask(ship->task_id)->target + 1) * 360);
...
void Instance::GenerateTasks()
{
	...
	for (auto& kv : ships) {
		...
		taskAttack->max_ships = -1;// TODO See
	}
